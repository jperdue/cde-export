...
namespace cde_web.Controllers
{
	public class HomeController : Controller
	{
		...
		List<Result> Test(Stream stream, string testName)
		{
			...
			foreach (var group in errors.GroupBy(e => e.Message).OrderBy(g => g.Key))
			{
				results.Add(new Result { Title = group.Key, Errors = group.Select(e => e.Row.ToString() ).ToList()});
			}

			var testTotals = errors.GroupBy(e => e.TestName).OrderBy(g => g.Key).Select(g => new Tuple<string, int>(g.Key, g.Count()));
